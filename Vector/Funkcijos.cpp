...
void DuomenuKurimas(int Failu_Kiekis)
{
	int n = 100;
	int Skaicius2 = 0;
...
void Skaitymas(vector<Universitetas>& Studentas, int & n, int Med_Vid, int TekstinioFailoNr)
{
	...
	while (in.peek() != EOF)
	{
		...
		if (PazymiuSk > 0 && PazymiuSk != 1)
		{
			...
			Studentas.push_back({ Vardas, Pavarde, Egzaminas, Galutinis, PazymiuSk });
			n++;
		}
		else
		{
...
bool PazymioIeskojimas(Universitetas& stud2)
{
	return stud2.Galutinis < 5; // pasibandyt su >
}



void Ilgiausias(vector<Universitetas>& A, int n, int &Vardas, int &Pavarde) // Funkcija ilgiausiam vardui ir pavardei rasti
{
	int MAX_V = 0;
	int MAX_PV = 0;
	int ilgis1 = 0;
...
void geriBlogiMokiniai(vector<Universitetas>& Studentas, int n, int Ilgiausias_Vardas, int Ilgiausia_Pavarde, int TekstinioFailoNr)
{
	...
	auto search = std::find_if(Studentas.begin(), Studentas.end(), PazymioIeskojimas);
	...
	for (int i = 0; i < Studentas.size(); i++)
		out1 << left << setw(Ilgiausias_Vardas) << Studentas[i].Vardas << " " << left << setw(Ilgiausia_Pavarde) << Studentas[i].Pavarde << fixed << setprecision(2) << "  " << Studentas[i].Galutinis << endl;
