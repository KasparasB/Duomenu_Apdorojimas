...
void Ilgiausias(deque<Universitetas>& A, int n, int &Vardas, int &Pavarde) // Funkcija ilgiausiam vardui ar pavardei rasti
{
	int MAX_V = 0;
	int MAX_PV = 0;
	int ilgis1 = 0;
...
void VardasPavarde(deque<Universitetas> &A, int i) // Vartotojas netgi turi galimybe istaisyti neteisinga varda tekstiniame faile :O
{
...
void geriBlogiMokiniai(deque<Universitetas>& Studentas, int n, int Ilgiausias_Vardas, int Ilgiausia_Pavarde, int TekstinioFailoNr)
{
	...
	deque<Universitetas> geri(Studentas.begin(), Studentas.begin() + slenkstis);
	deque<Universitetas> blogi(Studentas.begin() + slenkstis, Studentas.end());
